...
def to_postgres(**context):
    ...
    logger.info(
        f"Looking for files from today ({today}) in s3://{S3_BUCKET}/{today_prefix}"
    )
    ...
    if not keys:
        error_msg = (
            f"No hay archivo de hoy ({today}) en s3://{S3_BUCKET}/{today_prefix}"
        )
    ...
    if not json_keys:
        error_msg = (
            f"No hay archivo JSON de hoy ({today}) en s3://{S3_BUCKET}/{today_prefix}"
        )
        logger.warning(error_msg)
        raise ValueError(error_msg)
    ...
    logger.info(
        f"Successfully loaded {inserted_count} shows as raw JSON into PostgreSQL database"
    )
...
with DAG(
    ...
    with TaskGroup("sync") as sync_group:
